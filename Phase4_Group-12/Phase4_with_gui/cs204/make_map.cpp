...
string make_map:: reg_to_binary(string s) // eg: x9 => "01001"
{
	int s2;
	if(s.length()==2)
	{
...
string make_map:: funcP(string ins_name,string rd,int imm)
{
	string opcode = mopcode.find(ins_name)->second;
	string des = reg_to_binary(rd);
	bitset<20> res(imm);
...
void make_map:: create_map()
{
	...
	/*for P type    To handle pseudo instruction "la"   */
	mtype.insert( pair<string,string>("la","P") );
	mopcode.insert( pair<string,string>("la","0000000") );
	
	//as last instruction of machine code
	mtype.insert( pair<string,string>("swi","exit") ); // 0xEF000011  <=  swi 0x11
