...
void original:: compatible_original()
{
    ...
    //ofstream file3("original_code_for_qt.txt",ios::out);
		...
		while(original_codefp>>ins_name)
		{
			...
			else
			{
				...
				if(type=="R")
				{
					...
					file3<<ins_name<<" "<<rd<<" "<<rs1<<" "<<rs2<<endl;
					string rs_2;
					int i=0;
				...
				else if(type=="I")
				{
					...
					file3<<ins_name<<" "<<rd<<" "<<rs1<<" "<<imm<<endl;
					if ( label [label.length()-1] == ':')
					{
						mlabel.insert( pair<string,int>(label.substr(0,label.length()-1),4*countpc) );
					}
				...
				else if(type=="S")
				{
					...
					file3<<ins_name<<" "<<rs2<<" "<<rs1<<" "<<imm<<endl;
				}
				else if(type=="SB")
				{
					string imm ;
					...
					file3<<ins_name<<" "<<rs1<<" "<<rs2<<" "<<imm<<endl;
				}
				else if(type=="U")
				{
					string imm ;
